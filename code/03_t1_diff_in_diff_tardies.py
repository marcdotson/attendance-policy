#This code contains trimester 1 TARDY data diff-in-diff as well as creation of line charts to show trends
...
plt.title('Average Tardies Per Period')
plt.legend()
...
plt.xlabel('Period')
plt.ylabel('Proportion of Average Tardies')
plt.title('Average Tardies Proportion Per Period')
plt.legend()
plt.tight_layout()
plt.show()
...
plt.xlabel('Period')
plt.ylabel('Average Tardies / Difference')
plt.title('Average Tardies Per Period and SV-GC Differences')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
y = df_reg['avg_tardies']
lr.fit(X, y)
print(f'Coefficients: {lr.coef_}')
# Regression via statsmodels
ols_model = ols('avg_tardies ~ school + year + school_year', data=df_reg).fit()
print(ols_model.summary())

# Calculate DID manually
mean_gc_2023 = gc_2023_long['avg_tardies'].mean()
mean_sv_2023 = sv_2023_long['avg_tardies'].mean()
mean_gc_2024 = gc_2024_long['avg_tardies'].mean()
mean_sv_2024 = sv_2024_long['avg_tardies'].mean()
